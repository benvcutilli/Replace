...
# This file replaces the in-text keys from InText ^^^intext^^^. Please look at
...
# n holds the new keys, which will be generated from ^^^intext^^^'s
# tagcreator.py
n = {}

# This does the replacement of the key in all files, including ^^^intext^^^'s
...
# Path to tagcreator.py from ^^^intext^^^:
p = "tagcreator.py"
def newtag(capture):
    c = capture.group(1)
    if c not in n:
        # Calling ^^^intext^^^'s tagcreator.py in order to get a new key to swap
        # in for the old key. These keys are used in tags and keys, respectively
        # discussed in ^^^intext^^^'s intext.py's (C) and (A).
...
# The JSON file from (A) of intext.py ^^^intext^^^ needs to have replaced the
